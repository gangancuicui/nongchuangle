	...
	export default {
		...
		methods:{
			...
			select(e){
			    ...
			    const db =wx.cloud.database().collection('farmer').where({paperid:id, _openid:this.openid})
			    db.get({
			      success: (res)=>{
			        console.log(res);
			...
			getNewsList(){
				console.log('res')
				const db =uni.cloud.database().collection('farmer').where({paperid:"种植", _openid:this.openid})
				    db.get({
				      success: (res)=>{
				        console.log(res);
